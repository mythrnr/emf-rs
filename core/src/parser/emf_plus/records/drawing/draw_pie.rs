...
/// The EmfPlusDrawPie record specifies drawing a section of the
/// interior of an ellipse.
#[derive(Clone, Debug)]
pub struct EmfPlusDrawPie {
    /// Type (2 bytes): An unsigned integer that identifies this record
    /// type as EmfPlusDrawPie from the RecordType enumeration. The
    /// value MUST be 0x4011.
    ...
    /// EMF+ Object Table to draw the pie. The value MUST be zero to
    /// 63, inclusive.
    pub object_id: u8,
    ...
    /// extent of the arc that defines the pie wedge to draw, as an
...
impl EmfPlusDrawPie {
    ...
    pub fn parse<R: crate::Read>(
        ...
        crate::parser::ParseError::expect_eq(
            "record_type",
            record_type as u16,
            crate::parser::emf_plus::RecordType::EmfPlusDrawPie as u16,
        )?;
...
mod tests {
    ...
    #[test]
    fn parses_angles_and_a_compressed_rectangle() {
        let mut data = vec![];
        data.extend(90.0_f32.to_le_bytes());
        data.extend((-180.0_f32).to_le_bytes());
        for v in [1_i16, 2, 30, 40] {
            data.extend(v.to_le_bytes());
        }

        let mut buf: &[u8] = &data;
        let data_size = crate::parser::Size::from(data.len() as u32);
        let record = EmfPlusDrawPie::parse(
            &mut buf,
            crate::parser::emf_plus::RecordType::EmfPlusDrawPie,
            0x4001,
