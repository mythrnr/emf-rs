...
/// The EmfPlusDrawClosedCurve record specifies drawing a closed
/// cardinal spline.
#[derive(Clone, Debug)]
pub struct EmfPlusDrawClosedCurve {
    /// Type (2 bytes): An unsigned integer that identifies this record
    /// type as EmfPlusDrawClosedCurve from the RecordType enumeration.
    ...
    /// specifies compressed data. If set, PointData specifies
    /// locations in the coordinate space with 16-bit signed integer
    /// coordinates. If clear, PointData specifies locations in the
    ...
    /// EMF+ Object Table to draw the closed curve. The value MUST be
    ...
    /// 0x0000001C or greater; the minimum Size is computed as follows:
    ///
    ...
    /// field, the value MUST be 0x0000002C or greater; Size is
    /// computed as follows:
    ///
    ...
    ///
    /// If the P bit is set in the Flags field, the value MUST be
    /// 0x00000010 or greater; the minimum DataSize is computed as
    /// follows:
    ///
    ...
    ///
    /// If the P bit is clear and the C bit is clear in the Flags
    /// field, the value MUST be 0x00000020 or greater; DataSize is
    /// computed as follows:
    ///
    ...
    /// as follows:
    ///
    /// - EmfPlusPointR object: If the P bit is set in the Flags field, the
    ///   points specify relative locations.
    ...
    ///   field, the points specify absolute locations with 32-bit coordinates.
    pub point_data: crate::parser::emf_plus::objects::EmfPlusPoints,
}

impl EmfPlusDrawClosedCurve {
    ...
    pub fn parse<R: crate::Read>(
        ...
        crate::parser::ParseError::expect_eq(
            "record_type",
            record_type as u16,
            crate::parser::emf_plus::RecordType::EmfPlusDrawClosedCurve as u16,
        )?;
...
mod tests {
    ...
    fn parses_compressed_points_and_stores_the_header() {
        ...
        let record = EmfPlusDrawClosedCurve::parse(
            &mut buf,
            crate::parser::emf_plus::RecordType::EmfPlusDrawClosedCurve,
            0x4002,
        ...
        assert_eq!(record.record_type as u16, 0x4017);
        assert_eq!(record.flags, 0x4002);
