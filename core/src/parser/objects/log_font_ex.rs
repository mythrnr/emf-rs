...
impl LogFontEx {
    ...
    pub fn parse<R: crate::Read>(
        ...
        let full_name = read_bytes_field(buf, &mut consumed_bytes, 128)?;
        let style = read_bytes_field(buf, &mut consumed_bytes, 64)?;
        let script = read_bytes_field(buf, &mut consumed_bytes, 64)?;
