...
#[derive(Clone, Debug)]
pub struct EMR_DRAWESCAPE {
    /// Type (4 bytes): An unsigned integer that identifies this record type
    /// from the RecordType enumeration. It MUST be EMR_DRAWESCAPE, which is
    /// 0x00000069.
...
impl EMR_DRAWESCAPE {
    ...
    pub fn parse<R: std::io::Read>(
        ...
        if record_type != crate::parser::RecordType::EMR_DRAWESCAPE {
            return Err(crate::parser::ParseError::UnexpectedPattern {
                cause: format!(
                    "record_type must be `{:#010X}`, but specified `{:#010X}`",
                    crate::parser::RecordType::EMR_DRAWESCAPE as u32,
                    record_type as u32
                ),
            });
        }
