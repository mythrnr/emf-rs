...
#[derive(Clone, Debug)]
pub struct EMR_ABORTPATH {
    /// Type (4 bytes): An unsigned integer that identifies this record type
    /// from the RecordType enumeration. It MUST be EMR_ABORTPATH, which is
    /// 0x00000044.
...
impl EMR_ABORTPATH {
    ...
    pub fn parse<R: crate::Read>(
        ...
        if record_type != crate::parser::RecordType::EMR_ABORTPATH {
            return Err(crate::parser::ParseError::UnexpectedPattern {
                cause: format!(
                    "record_type must be `{:#010X}`, but specified `{:#010X}`",
                    crate::parser::RecordType::EMR_ABORTPATH as u32,
                    record_type as u32
                ),
            });
        }
