...
///
/// The pattern brush object defined by this record can be selected into the
/// playback device context by an EMR_SELECTOBJECT record, which specifies the
/// pattern brush to use in subsequent graphics operations.
#[derive(Clone, Debug)]
pub struct EMR_CREATEDIBPATTERNBRUSHPT {
    ...
    /// pattern brush object in the EMF object table. This index MUST be saved
    ...
    /// of a DeviceIndependentBitmap object ([MS-WMF] section 2.2.2.9). It is
    /// not required to be contiguous with the fixed portion of this record.
    ///
...
impl EMR_CREATEDIBPATTERNBRUSHPT {
    #[tracing::instrument(
